...
exports.GetName = async (req, res) => {
    try {
        ...
        const nameTitleList = titlePrincipals ? await Promise.all(
            titlePrincipals.map(async (nameTitle) => {
                const title = await TitleBasics.findOne({tconst: nameTitle.tconst}).exec()
                return {
